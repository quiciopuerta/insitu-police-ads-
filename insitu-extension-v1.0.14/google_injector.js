...
(function () {
  ...
  function attachBudgetValidator(input) {
    ...
    const validateBudget = () => {
      ...
      if (numValue && numValue > currentBudgetLimit) {
        ...
        infoBadge.textContent = `❌ ERROR: Límite excedido (Max: $${currentBudgetLimit})`;
        ...
        const rect = input.getBoundingClientRect();
        infoBadge.textContent = `✅ PRESUPUESTO SEGURO`;
    ...
    input.addEventListener('blur', () => {
      ...
      if (numValue && numValue > currentBudgetLimit) {
        ...
        triggerGoogleBudgetAlertOverlay(numValue, currentBudgetLimit, input);
        
        // Alerta nativa
        alert(`🚨 ERROR DE PRESUPUESTO - GOOGLE ADS 🚨\n\nEl presupuesto diario configurado ($${numValue} USD) de Google Ads supera el Límite Máximo Seguro de Gobernanza establecido por insitu.company ($${currentBudgetLimit} USD).\n\nPor favor, corrige el valor antes de publicar la campaña.`);
      }
    });
  }
   ...
   */
  function triggerGoogleBudgetAlertOverlay(writtenValue, limit, originalInput) {
      ...
      <div style="display: flex; align-items: flex-start; gap: 12px;">
        ...
        <div>
          ...
          <p style="font-size: 12px; margin: 0 0 10px 0; line-height: 1.4;">
            El presupuesto diario de Google Ads ingresado (<strong>$${writtenValue} USD</strong>) supera el tope de seguridad de esta cuenta (<strong>$${limit} USD</strong>).
          </p>
