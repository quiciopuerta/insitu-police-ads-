...
(function () {
  ...
  function attachBudgetValidator(input) {
    ...
    input.addEventListener('blur', () => {
      ...
      if (numValue && numValue > currentBudgetLimit) {
        ...
        triggerBudgetAlertOverlay(numValue, currentBudgetLimit, input);
        
        // Alerta nativa para bloquear la interacción insegura temporalmente
        alert(`🚨 ERROR DE PRESUPUESTO - insitu.company 🚨\n\nEl presupuesto diario configurado ($${numValue} USD) supera el Límite Máximo Seguro de Gobernanza establecido en esta cuenta ($${currentBudgetLimit} USD).\n\nPor favor, corrige el valor antes de continuar.`);
      }
    });
  }
   ...
   */
  function triggerBudgetAlertOverlay(writtenValue, limit, originalInput) {
