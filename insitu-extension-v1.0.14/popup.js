  ...
  const initializeMainApp = () => {
    ...
    const updateOutput = () => {
      ...
      chrome.storage.local.get({ maxBudget: 500 }, (res) => {
        ...
        if (window.InsituValidator) {
          urlOutput.textContent = window.InsituValidator.buildUtmUrl(baseUrlInput.value, country, channel, objective, product, season);
        }
      });
    };

    [inputCountry, inputChannel, inputObjective, inputProduct, inputSeason, baseUrlInput, inputCampaignBudget].forEach(el => {
